  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      if (isAssigning) {
        ...
        if (newQty <= 0) {
          setTimeout(() => toast.error(`${editForm.name.trim()} is now out of stock.`), 800)
        } else if (newQty <= editTarget.low_stock_threshold) {
          setTimeout(() => toast.warning(`${editForm.name.trim()} is low on stock (${newQty} remaining).`), 800)
        }
      }
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="w-[calc(36rem-110px)] max-w-none sm:max-w-none">
          <DialogHeader>
